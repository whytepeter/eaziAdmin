...
export default {
  ...
  setup(props, { emit }) {
    ...
    const payload = reactive({
      ...
      sort: {
        createdAt: undefined,
        durationEndedAt: undefined,
        loanAmountApplied: undefined,
    ...
    const filterFuction = async (arg) => {
      if (arg.type == 'status' || arg.type == 'type' || arg.type == 'overdue') {
        ...
        clearSort()
        payload.sort[arg.value] = arg.order
      } else if (arg.type == 'name') {
        clearSort()
        payload.sort.companyId.company[arg.value] = arg.order
      }
      emit('filter', payload)
