<template>
  <div>
    <h3 class="mb-6 font-normal">Admin(s)</h3>
      ...
      <div>
        <div class="flex items-start gap-8 admin-list-layout">
          <div class="w-3/12 h-full flex flex-col overflow-hidden">
            <employee-list v-model:selected="selectedUser" class="bg-white rounded-md p-3" :data="data.companyAdmins">
          ...
          <div class="w-9/12 h-full">
            <div class="card card-white h-full" v-if="selectedUser">
              <div class="flex gap-8">
                <picture class="flex-shrink-0 h-98 w-72 rounded-lg overflow-hidden">
                  <img :src="selectedUser.pictureUrl" class="h-full w-full object-cover" v-if="selectedUser.pictureUrl" />
                  <div v-else class="bg-gray-300 h-full"></div>
                </picture>
                ...
                <div class="flex-1">
                  <div>
                    ...
                    <div class="grid grid-cols-2 gap-8 gap-y-10">
                      ...
                      <copyable-value-card noPadding label="Phone Number" :value="selectedUser.phoneNumber">
                        <template #icon>
                          <CallIcon />
                        </template>
                      </copyable-value-card>
...
const { data, error } = useQuery('ListCompanyAdmins', {
  companyId: companyId
})
...
watchEffect(() => {
  if (data.value && !selectedUser.value) {
    const { companyAdmins } = data.value
    if (companyAdmins && companyAdmins.length > 0) {
      selectedUser.value = companyAdmins[0]
    }
  }
})
</script>
